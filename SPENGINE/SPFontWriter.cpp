...
namespace SPEngine
{
	...
	bool SPFontWriter::WriteText( SPTextToWrite text )
	{
		...
		SPDevice::GetSingleton().GetD3DDevice()->Clear(1, NULL, D3DCLEAR_TARGET, 0x00000000, 1.0, 0);
		...
		if (text.pixelShader && text.pixelShader->GetCore())
		{
			...
			SPDevice::GetSingleton().GetD3DDevice()->Clear(1, NULL, D3DCLEAR_TARGET, 0, 1.0, 0);
		...
		SPTexturePtr target)
	{
		...
		//	text, count, destRect, format, color, depth, target);
